...
bool solveSudoku(vector<vector<int>>& g, vector<Cell>& b);

/*makes a copy of the puzzle and vector of remaining cells.
Tries to solve the puzzle with that copy. If the number we guessed ends
up solving the puzzle then return true.*/
bool guess(vector<vector<int>> g, vector<Cell> b, int r, int c, int s, int n){
	g[r][c] = n;
	update(b, r, c, s, n);
	if (solveSudoku(g, b)) return true;
	return false;
}
...
bool solveSudoku(vector<vector<int>>& g, vector<Cell>& b){
	...
	while (!b.empty()){
		...
		if (!b.empty() && changed == false){ //make sure that guessing is necessaryand that you didnt already finish the puzzle
			for (int i = 0; i < b[0].possibilities.size(); ++i){
				int r = b[0].row, c = b[0].col, s = b[0].sec, num = b[0].possibilities[i];
				if (guess(g, b, r, c, s, num)){ //if the number helps us solve the puzzle then use it
					g[r][c] = num;
					update(b, r, c, s, num);
					i = 10;
					changed = true;
...
int main(int argc, const char * argv[]) {
	...
	/*When I try to implement guessing it solves this puzzle incorrectly and it leaves some spots empty*/
	cout << "\nPuzzle 50" << endl;
